...
# ---------------------------------------------------------------------------
# TEMPORARY CLEANUP: Delete bizengo4smes@gmail.com and its transactions
# ---------------------------------------------------------------------------
_BIZENGO_SECRET = "siiqo-delete-bizengo-2026-vB7"

@admin_bp.route('/cleanup-bizengo', methods=['POST'])
def cleanup_bizengo():
    secret = request.headers.get('X-Cleanup-Secret', '')
    if secret != _BIZENGO_SECRET:
    ...
    try:
        ...
        from app.models.communication import Notification, Message
        from app.models.community import Review
        from app.models.withdrawal import DayaPayment, PODPayment, VendorBankAccount, VendorCryptoWallet
        from app.models.payment_link import PaymentLink
        from app.models.event import Event, TicketType, TicketPurchase

        target_email = "bizengo4smes@gmail.com"
        user = User.query.filter(User.email.ilike(target_email)).first()

        if not user:
            orphan_orders = Order.query.filter(Order.buyer_email.ilike(target_email)).all()
            return jsonify({
                "message": "User not found with email bizengo4smes@gmail.com",
        ...
        user_orders = Order.query.filter(
            ...
            Order.buyer_email.ilike(target_email)
        ).all()
        order_ids = [o.id for o in user_orders]

        paylinks = PaymentLink.query.filter_by(vendor_id=user.id).all()
        ...
        audit = {
            "user_id": user.id,
            "email": user.email,
            "fullname": getattr(user, 'fullname', getattr(user, 'name', 'N/A')),
            "storefront": user.storefront.store_name if user.storefront else None,
            "orders_count": len(user_orders),
            "payment_links_count": len(paylinks),
        ...
        # EXECUTION
        if order_ids:
            TicketPurchase.query.filter(TicketPurchase.order_id.in_(order_ids)).delete(synchronize_session=False)
        TicketPurchase.query.filter((TicketPurchase.buyer_id == user.id) | TicketPurchase.buyer_email.ilike(target_email)).delete(synchronize_session=False)

        if order_ids:
            PODPayment.query.filter(PODPayment.order_id.in_(order_ids)).delete(synchronize_session=False)
            DayaPayment.query.filter(DayaPayment.order_id.in_(order_ids)).delete(synchronize_session=False)
            LogisticsAssignment.query.filter(LogisticsAssignment.order_id.in_(order_ids)).delete(synchronize_session=False)
            Review.query.filter(Review.order_id.in_(order_ids)).delete(synchronize_session=False)
            Notification.query.filter(Notification.order_id.in_(order_ids)).update({"order_id": None}, synchronize_session=False)
            Message.query.filter(Message.order_id.in_(order_ids)).update({"order_id": None}, synchronize_session=False)
            Invoice.query.filter(Invoice.order_id.in_(order_ids)).delete(synchronize_session=False)
            Receipt.query.filter(Receipt.order_id.in_(order_ids)).delete(synchronize_session=False)
            EscrowTransaction.query.filter(EscrowTransaction.order_id.in_(order_ids)).delete(synchronize_session=False)
        ...
        Invoice.query.filter(Invoice.vendor_id == user.id).delete(synchronize_session=False)
        Receipt.query.filter(Receipt.vendor_id == user.id).delete(synchronize_session=False)

        for pl in paylinks:
            db.session.delete(pl)

        for ev in events:
            TicketPurchase.query.filter_by(event_id=ev.id).delete(synchronize_session=False)
        ...
        Review.query.filter((Review.buyer_id == user.id) | (Review.vendor_id == user.id)).delete(synchronize_session=False)
        ...
        return jsonify({
            "status": "SUCCESS",
            "message": f"User {target_email} and all associated data deleted successfully.",
            "deleted_orders": len(user_orders),
            "deleted_paylinks": len(paylinks),
        ...
        logging.error(f"[ADMIN][CLEANUP_BIZENGO] Error: {tb}")
